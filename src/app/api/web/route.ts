...
import Web from '@/src/models/web'
...
export async function GET(request: NextRequest) {
  try {
    ...
    const topics = await Web.find(query)
    return NextResponse.json({ topics }, { status: 200 })
  } catch (error) {
    console.error('Error in GET /api/web:', error)
...
export async function POST(request: NextRequest) {
  try {
    ...
    const newTopic = new Web({ title, description })
    ...
    console.error('Error in POST /api/web:', errorMessage)
